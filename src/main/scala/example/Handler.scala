...
class Handler {
  ...
  private def get_actions(wumpusPercept: WumpusPercept) = {
    Console.println("I am in position " + agentX + "-" + agentY + "")
    ...
    if (room_info.isEmpty) {
      Console.println("I'm sorry, there's nothing here")
    }
    else {
      Console.println("")
    }
    if (actions == null) {
      ...
      if (imaginatyWorld.isWampusAlive && imaginatyWorld.wampusRoomCount > 2) {
        ...
        Console.println("I'm go to: " + nextOkRooms(best_candidate).x + "-" + nextOkRooms(best_candidate).y)
        actions = getNextRoomAction(agentPosition, nextOkRooms(best_candidate), Forward)
        Console.println("and do " + actions._1)
      }
    }
